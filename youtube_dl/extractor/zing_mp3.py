...
class Zing_mp3_vnIE(InfoExtractor):
    _VALID_URL = r'''(?x)^
        (?:https?://|//)(?:www\.|m\.|)
        (?P<site>
            (zingmp3\.vn)
...
class Zing_mp3_vnPlaylistIE(Zing_mp3_vnIE):
    IE_NAME = "Zing_mp3_vn:playlist"
    ...
    def _entries_for_the_loai_video(self, id_the_loai_video):
        self.IE_NAME = "Zing_mp3_vn:playlist-video"
    ...
    def _entries_for_chu_de(self, id_chu_de):
        self.IE_NAME = "Zing_mp3_vn:chu-de"
...
class Zing_mp3_vnChartIE(Zing_mp3_vnIE):
    IE_NAME = "Zing_mp3_vn:#zingchart"
    ...
    def _entries(self, items):
        for item in items:
            ...
            url = compat_urllib_parse.urljoin(self._default_host, item.get('link'))
            video_id = item.get('id')
            yield self.url_result(url, ie=Zing_mp3_vnIE.ie_key(), video_id=video_id)


class Zing_mp3_vnUserIE(Zing_mp3_vnIE):
    _VALID_URL = r'''(?x)^
        (?:https?://|//)(?:www\.|m\.|)
        (?P<site>
            (zingmp3\.vn)

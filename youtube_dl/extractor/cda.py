...
class CDAIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        view_count = self._search_regex(r'Odsłony:(?:\s|&nbsp;)*([0-9]+)', webpage,
                                        'view_count', default=None)
        average_rating = self._search_regex(r'<(?:span|meta)[^>]*itemprop=(["\'])ratingValue\1[^>]*>(?P<rating_value>[0-9.]+)',
        ...
        for href, resolution in re.findall(
                ...
                webpage):
            webpage = self._download_webpage(
                self._BASE_URL + href, video_id, 'Downloading %s version information' % resolution, fatal=False)
            if not webpage:

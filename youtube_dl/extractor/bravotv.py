...
class BravoTVIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        account_pid, release_pid, release_pid2 = [None] * 3
        tve = settings.get('ls_tve')
        if tve:
            query['manifest'] = 'm3u'
            account_pid = 'HNK2IC'
            release_pid = tve['release_pid']
            release_pid2 = self._search_regex(r'<div[^>]+pdk-player[^>]+data-url="[^\?]+/([^\?/]+)\?', webpage, 'pdk-player')
            if tve.get('entitlement') == 'auth':
                adobe_pass = settings.get('tve_adobe_auth', {})
                resource = self._get_mvpd_resource(
                    adobe_pass.get('adobePassResourceId', 'bravo'),
                    tve['title'], release_pid2 if release_pid2 else release_pid, tve.get('rating'))
                query['auth'] = self._extract_mvpd_auth(
                    url, release_pid2 if release_pid2 else release_pid, adobe_pass.get('adobePassRequestorId', 'bravo'), resource)
        else:
        ...
        info.update({
            ...
            'id': release_pid2 if release_pid2 else release_pid,
            'url': smuggle_url(update_url_query(
                'http://link.theplatform.com/s/%s/media/%s' % (account_pid, release_pid2),
                query), {'force_smil_url': True}) if release_pid2 else smuggle_url(update_url_query(
                'http://link.theplatform.com/s/%s/%s' % (account_pid, release_pid),
                query), {'force_smil_url': True}) if release_pid else ls_playlist['player_base_url'],
            'ie_key': 'ThePlatform',
        })
        return info

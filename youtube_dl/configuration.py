...
from .utils import get_term_width, get_cachedir, write_string
from .version import __version__


def parse_options(override_with=None):
    # No need to wrap help messages if we're on a wide console
    columns = get_term_width()
    max_width = columns or 80
    max_help_position = 80

    fmt = optparse.IndentedHelpFormatter(width=max_width, max_help_position=max_help_position)
    fmt.format_option_strings = _format_option_string

    parser = optparse.OptionParser(version=__version__, formatter=fmt, usage='%prog [options] url [url...]', conflict_handler='resolve')
    ...
    for group in general, selection, downloader, filesystem, verbosity, video_format, subtitles, authentication, postproc:
        parser.add_option_group(group)

    if override_with is not None:
        opts, args = parser.parse_args(override_with)
        if opts.verbose:
            write_string(u'[debug] Override config: ' + repr(override_with) + '\n')
    else:
        cmdline_config = sys.argv[1:]
        if '--ignore-config' in cmdline_config:
            system_config = []
            user_config = []
        else:
            system_config = _read_options('/etc/youtube-dl.conf')
            if '--ignore-config' in system_config:
                user_config = []
            else:
                user_config = _read_user_config()
        argv = system_config + user_config + cmdline_config

        opts, args = parser.parse_args(argv)
        if opts.verbose:
            write_string(u'[debug] System config: ' + repr(_hide_login_info(system_config)) + '\n')
            write_string(u'[debug] User config: ' + repr(_hide_login_info(user_config)) + '\n')
            write_string(u'[debug] Command-line args: ' + repr(_hide_login_info(cmdline_config)) + '\n')
...
def _read_options(filename_bytes, default=[]):
    try:
        with open(filename_bytes) as option_file:
            return [shlex.split(line, comments=True) for line in option_file]
    except IOError:
        return default
...
def _read_user_config():
    ...
    if xdg_config_home:
        config_file = os.path.join(xdg_config_home, 'youtube-dl', 'config')
        if not os.path.isfile(config_file):
            config_file = os.path.join(xdg_config_home, 'youtube-dl.conf')
    else:
        config_file = os.path.join(os.path.expanduser('~'), '.config', 'youtube-dl', 'config')
        if not os.path.isfile(config_file):
            config_file = os.path.join(os.path.expanduser('~'), '.config', 'youtube-dl.conf')
    user_config = _read_options(config_file, None)

    if user_config is None:
        appdata_dir = os.environ.get('appdata')
        if appdata_dir:
            user_config = _read_options(os.path.join(appdata_dir, 'youtube-dl', 'config'), default=None)
            if user_config is None:
                user_config = _read_options(os.path.join(appdata_dir, 'youtube-dl', 'config.txt'), default=None)

    if user_config is None:
        user_config = _read_options(os.path.join(os.path.expanduser('~'), 'youtube-dl.conf'), default=None)
    if user_config is None:
        user_config = _read_options(os.path.join(os.path.expanduser('~'), 'youtube-dl.conf.txt'), default=None)
...
def _format_option_string(option):
    ...
    if option._short_opts:
        options.append(option._short_opts[0])
    if option._long_opts:
        options.append(option._long_opts[0])
    if len(options) > 1:
        options.insert(1, ', ')

    if option.takes_value():
        options.append(' %s' % option.metavar)
...
def _hide_login_info(options):
    private_options = set(['-p', '--password', '-u', '--username', '--video-password'])
    options = list(options)
    for i, option in enumerate(options):
        try:
            if option in private_options:
                options[i + 1] = '<PRIVATE>'
        except ValueError:
            pass
    return options
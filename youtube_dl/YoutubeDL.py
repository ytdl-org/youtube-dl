    ...
    def process_subtitles(self, video_id, normal_subtitles, automatic_captions):
        ...
        for lang in requested_langs:
            ...
            for ext in formats_preference:
                if ext == 'best':
                    ...
                    if formats_auto:
                        fa = formats_auto[-1]
                    break
                if formats and not f:
                    matches = list(filter(lambda f: f['ext'] == ext, formats))
                    if matches:
                        f = matches[-1]
                if formats_auto and not fa:
            ...
            if formats_auto and not fa:
                fa = formats_auto[-1]
                self.report_warning(
                    'No automatic subtitle format found matching "%s" for language %s, '
                    'using %s' % (formats_query, lang, fa['ext']))
            if formats and formats_auto:
                f = copy.deepcopy(f)
                f.update({'auto': fa})
            elif formats_auto:
                f = fa

...
Further note that if you attempt to convert the downloaded video files to audio-only using `-x` (and without `-k`), youtube-dl uses `-f bestaudio/best` as the default format selection to only download audio files if available.

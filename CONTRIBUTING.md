...
* ``re.search`` &rightarrow; ``self._parse_regex``
* ``json.loads`` &rightarrow; ``self._parse_json``
* ``libxml2.parseMemory`` &rightarrow; ``self._parse_xml``
* ``libxml2.xpathEval`` &rightarrow; ``xpath_with_ns``, ``xpath_element``, ``xpath_text``
